...
- `id` は空文字不可。
- `name` は trim 後に空文字不可。
- `gender` は `female` / `male` のいずれか。`matchupMode` が `sameGenderPriority` / `mixedDoublesPriority` の場合は必須。
...
| `participants[].gender` | string | conditional | `female` / `male`。`matchupMode` が `standard` 以外の場合は必須 |
